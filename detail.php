<?php require_once("config.php"); ?>
<!DOCTYPE html>
<html>
<head>
	<title></title>
	<title> Lazodi - Better at Prices</title>
	<link href="css/bootstrap.min.css" rel="stylesheet">
	<meta name='viewport' content='width=device-width,initial-scale=1.0'>
</head>
<body>
	<div class="col-md-6 col-md-offset-3">
<a href=daker.php><input type=button value ='Kembali ke Menu' class='btn btn-primary' style='width:100%'></a>
		<br>
		<br>
	<div id="container">
		<div id="content">
			<div class="title"><label>Detail Keranjang Belanja</label></div>
			<table border="0" width="100%" cellspacing="0" cellpadding="3">
				<tr style="background-color: #DDD;">
					<th>No.</th>
					<th>Nama Barang</th>
					<th>Jumlah</th>
					<th>Harga Satuan</th>
					<th>Sub Total</th>
					<th>Opsi</th>
				</tr>
			    <?php
				//MENAMPILKAN DETAIL KERANJANG BELANJA//
				$con = mysqli_connect("localhost","root","","isep") or die ("Koneksi Gagal");
				$no = 1;
				foreach($_SESSION as $name => $value){
					if($value > 0){
						if(substr($name, 0, 5) == 'cart_'){
							$id = substr($name, 5, (strlen($name)-5));
							$get = mysqli_query($con,'SELECT * FROM twproduk WHERE produk_id='.mysqli_real_escape_string((int)$id));
							
							while($get_row = mysqli_fetch_assoc($get)){
								if($no % 2 == 0){
									$warna = "#EAEAEA";
								} else {
									$warna = "#F4F4F4";
								}
								$sub = $get_row['produk_harga'] * $value;
								echo '
								<tr bgcolor="'.$warna.'">
									<td align="center">'.$no.'</td>
									<td align="center">'.$get_row['produk_nama'].'</td>
									<td align="center">'.$value.'</td>
									<td align="right">Rp. '.$get_row['produk_harga'].'</td>
									<td align="right">Rp. '.$sub.'</td>
									<td align="center">
										<a href="detail.php?remove='.$id.'">[-]</a> 
										<a href="detail.php?add='.$id.'">[+]</a> 
										<a href="detail.php?delete='.$id.'" onclick="return confirm(\'Anda Yakin?\');">[x]</a><br>
									</td>
								</tr>							
								';
								$no++;
							}
							$total += $sub;
						}
					}
				}
				if($total == 0){
					echo '<tr><td colspan="5" align="center">Keranjang belanja masih kosong!</td></tr></table>';
					echo '<p><div align="right">
						<a href="daker.php"><button class="btn btn-primary"> Lanjutkan Belanja</button></a>
						</div></p>';
				} else {
					echo '
						<tr style="background-color: #DDD;"><td colspan="4" align="right"><b>Total :</b></td><td align="right"><b>Rp. '.$total.'</b></td></td></td><td></td></tr></table>
						<p><div align="right">
						<a href="daker.php"><button class="btn btn-primary">Lanjutkan Belanja</button></a>
						<a href="checkout.php?total='.$total.'"><button class="btn btn-primary">Checkout </button></a>
						</div></p>
					';
				}
				?>
			
				<?php
				//PROSES TAMBAH JUMLAH PRODUK//
				if(isset($_GET['add'])){
					$qt = mysqli_query('SELECT produk_id, produk_jumlah FROM twproduk WHERE produk_id='.mysqli_real_escape_string((int)$_GET['add']));
					while($qt_row = mysqli_fetch_assoc($qt)){
						if($qt_row['produk_jumlah'] != $_SESSION['cart_'.$_GET['add']]){
							$_SESSION['cart_'.$_GET['add']]+='1';
							header("Location: detail.php");
						} else {
							echo '<script language="javascript">alert("Stok barang tidak mencukupi"); document.location="detail.php";</script>';
						}
					}
				}
				
				//PROSES HAPUS 1 ITEM PRODUK//
				if(isset($_GET['remove'])){
					$_SESSION['cart_'.$_GET['remove']]--;
					header("Location: detail.php");
				}
				
				//PROSES HAPUS SEMUA ITEM PRODUK//
				if(isset($_GET['delete'])){
					$_SESSION['cart_'.$_GET['delete']]='0';
					header("Location: detail.php");
				}
				?>
			</table>
		</div>

		<div class="clear"></div>

	</div>
	</div>
</body>
</html>